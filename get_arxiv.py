...
Article = namedtuple('Article', ['id', 'title', 'author', 'abstract', 'venue', 'url', 'categories', 'created'])
...
def pulp_xml_article(a, fp) :
    ...
    <created>%s</created>
</article>""" % (id_counter, a.title, a.author, a.abstract, a.venue, a.url, a.categories, a.created)
    ...
    while True :
        ...
        for record in dom.getElementsByTagName("record") :
            try :
                ...
                author = ", ".join(authors)
                articles.append(Article(id, title, author, abstract, venue, url, categories, created))
